...
def make_command(env, bin_path, num_ranks, **kwargs):
    if num_ranks in (None, 1):
        return bin_path
    else:
        mpiexec = multiget((env,), 'CXXTEST_MPIEXEC', 'mpiexec')
        np = multiget((env,), 'CXXTEST_MPI_NP', '-np')
        return '%s %s %d %s'%(mpiexec, np, num_ranks, bin_path)

def UnitTest(env, target, source = [], ranks=[], **kwargs):
    """
    ...
    # Handle the ranks set.
    if not ranks:
        ranks = [1]
    for num_ranks in ranks:
        cmd = make_command(env, test[0].abspath, num_ranks, **kwargs)
        if multiget([kwargs, env, os.environ], 'CXXTEST_SKIP_ERRORS', False):
            runner = env.Action(cmd, exitstatfunc=lambda x:0)
        else:
            runner = env.Action(cmd)
...
def generate(env, **kwargs):
    ...
    hdr_path = findCxxTestHeaders(env)
    if hdr_path:
        env.AppendUnique( CXXTEST_CPPPATH = [hdr_path]  )
    ...
    def CxxTest(env, target, source = None, ranks=[], **kwargs):
        """Usage:
        ...
        return UnitTest(env, target, source = deps, ranks=ranks, **kwargs)
